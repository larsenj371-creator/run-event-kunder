...
async function upsertCustomerForBooking({
  ...
  // Marketing consent is a separate, opt-in checkbox in the form — GDPR
  // doesn't allow bundling it with the (required) data-processing consent
  // needed to handle the booking itself. Only include emailMarketingConsent
  // in the mutation when the box was actually checked: omitting it entirely
  // when unchecked avoids downgrading a returning customer who is already
  // subscribed via some other channel.
  const emailMarketingConsent = marketingConsent
  ...
  if (existing) {
    ...
    let result = await graphql(UPDATE_CUSTOMER, {
      input: { id: customerId, tags, phone: normalizedPhone, emailMarketingConsent },
    });
    if (result.customerUpdate.userErrors.length && normalizedPhone && isInvalidPhoneError(result.customerUpdate.userErrors)) {
      ...
      result = await graphql(UPDATE_CUSTOMER, { input: { id: customerId, tags, phone: undefined, emailMarketingConsent } });
    }
    ...
    if (result.customerCreate.userErrors.length && normalizedPhone && isInvalidPhoneError(result.customerCreate.userErrors)) {
      result = await graphql(CREATE_CUSTOMER, {
        input: { firstName, lastName, email, phone: undefined, tags: createTags, emailMarketingConsent },
      });
    }
