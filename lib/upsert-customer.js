...
async function upsertCustomerForBooking({ name, email, phone, eventId, eventTitle, slotLabel }) {
  ...
  if (existing) {
    ...
    if (result.customerCreate.userErrors.length) {
      ...
      if (taken) {
        ...
        return upsertCustomerForBooking({ name, email, phone, eventId, eventTitle, slotLabel });
      }
